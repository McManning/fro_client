...
static int LoadBMP_RW (SDL_RWops *src, int freesrc, IMG_File* dst)
{
	int was_error;
	long fp_offset;
	int bmpPitch;
	...
	dst->format = IMG_FORMAT_BMP;
	return IMG_SurfaceToFrameset(surface, dst);
}

/* Load a BMP type image from an SDL datasource */
int IMG_LoadBMP_RW(SDL_RWops *src, IMG_File* dst)
{
	return(LoadBMP_RW(src, 0, dst));
}
