...
/* $Id: SDL_ttf.c,v 1.4 2003/11/06 18:46:34 eduardobrasao Exp $ */
...
UNICODE_strlen(const Uint16 * text)
{
    int size = 0;
...
UNICODE_strcpy(Uint16 * dst, const Uint16 * src, int swap)
{
    if(swap)
    {
        while(*src)
 ...
 */
void
TTF_ByteSwappedUNICODE(int swapped)
{
    TTF_byteswapped = swapped;
}

static void
TTF_SetFTError(const char *msg, FT_Error error)
{
...
static Uint16 *
LATIN1_to_UNICODE(Uint16 * unicode, const char *text, int len)
{
    int i;
...
int
TTF_GlyphMetrics(TTF_Font * font, Uint16 ch,
...
TTF_SizeText(TTF_Font * font, const char *text, int *w, int *h)
{
    ...
    unicode_text =
        (Uint16 *) ALLOCA((1 + unicode_len + 1) * (sizeof *unicode_text));
    ...
    for(ch = text; *ch; ++ch)
    {
        ...
        for(row = 0; row < current->rows; ++row)
        {
            ...
            dst = (Uint8 *) textbuf->pixels +
                (row + glyph->yoffset) * textbuf->pitch + xstart + glyph->minx;
...
*/
SDL_Surface *
TTF_RenderText_Shaded(TTF_Font * font,
...
*/
SDL_Surface *
TTF_RenderUTF8_Shaded(TTF_Font * font,
                      const char *text, SDL_Color fg, SDL_Color bg)
{
    SDL_Surface *textbuf;
    Uint16 *unicode_text;
...
SDL_Surface *
TTF_RenderUNICODE_Shaded(TTF_Font * font,
