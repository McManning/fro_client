...
void MD5::update (const char *input, unsigned int input_length) {
  ...
  if (input_length >= buffer_space) { // ie. we have enough to fill the buffer
    ...
    memcpy (buffer + buffer_index, (uint1*)input, buffer_space);
    transform (buffer);

    // now, transform each 64-byte piece of the input, bypassing the buffer
    for (input_index = buffer_space; input_index + 63 < input_length;
	 input_index += 64)
      transform ((uint1*)input+input_index);
  ...
  memcpy(buffer+buffer_index, (uint1*)input+input_index, input_length-input_index);
}
